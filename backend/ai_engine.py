...
class AISurvivalEngine:
    ...
    def simulate_step(self):
        ...
        if status.food_stability < 50:
            ...
            status.crew_count = max(1, status.crew_count) # 模拟人员变动或配给调整
        ...
        if status.energy_level < 30:
            ...
            status.oxygen_level = max(0, status.oxygen_level - 0.1) # 能源不足影响环控
            if status.energy_level < 10: emergency_mode = True

        # 6. 执行多系统联动逻辑（确保 AI 决策真正生效）
        ...
        if status.radiation_level > 50:
            ...
            status.food_stability -= protection_boost * 0.5  # 牺牲部分食物保鲜
            event_log.append(f"辐射联动：已启动地下储藏模式，医疗区防护+{protection_boost:.1f}%")
        
        # 食物联动：食物短缺时调整配给
        if status.food_stability < 30:
            status.crew_count = max(1, status.crew_count - 1)  # 模拟减少配给
        ...
        # 5. 调用 GLM-4-AIR 进行真实 AI 决策并执行联动
        ai_advice = "系统运行稳定"
        ai_action_taken = []  # 记录 AI 实际采取的行动
        ...
        try:
            ...
            if any(keyword in advice_lower for keyword in ['降低冷却', '减少制冷', '关闭非必要', '降低功耗']):
                status.food_stability -= 2.5  # 食物保鲜度下降
                status.energy_level += 3.0    # 节省能源
        ...
        return {
            ...
            "predictions": [max(0, status.survival_index - (d * 0.3)) for d in [30, 60, 90, 120]],
